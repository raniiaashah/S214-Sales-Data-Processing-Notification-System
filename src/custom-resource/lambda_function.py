...
def lambda_handler(event: dict, context: dict) -> None:
    ...
    logger.info(f"Resource Properties: {json.dumps(event.get('ResourceProperties', {}))}")
    logger.info("=" * 60)
    ...
    try:
        if request_type == 'Create':
            ...
            response_data, physical_id = handle_update(resource_properties, physical_resource_id)
