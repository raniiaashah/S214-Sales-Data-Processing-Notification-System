...
class TestSendReport:
    ...
    def test_send_report_with_custom_subject(self, notification_service, mock_sns_client):
    ...
    def test_send_report_long_subject_truncated(self, notification_service, mock_sns_client):
        """Test that long subjects are truncated to 100 characters."""
        mock_sns_client.publish.return_value = {
            'MessageId': 'test-message-id-012'
        }
    ...
    def test_send_report_includes_message_attributes(self, notification_service, mock_sns_client):
        """Test that report includes message attributes."""
        mock_sns_client.publish.return_value = {
            'MessageId': 'test-message-id-345'
        }
        ...
        assert call_args[1]['MessageAttributes']['report_type']['StringValue'] == 'daily_sales_report'


class TestSendErrorNotification:
    """Tests for send_error_notification method."""

    def test_send_error_notification_success(self, notification_service, mock_sns_client):
        """Test successful error notification sending."""
        mock_sns_client.publish.return_value = {
            'MessageId': 'error-message-id-123'
        }
    ...
    def test_send_error_notification_includes_environment(self, notification_service, mock_sns_client):
        """Test that error notification includes environment."""
        mock_sns_client.publish.return_value = {
            'MessageId': 'error-message-id-456'
        }
    ...
    def test_send_error_notification_includes_error_details(self, notification_service, mock_sns_client):
        """Test that error notification includes error details."""
        mock_sns_client.publish.return_value = {
            'MessageId': 'error-message-id-789'
        }
    ...
    def test_send_error_notification_sns_error(self, notification_service, mock_sns_client):
        """Test error notification with SNS error."""
        mock_sns_client.publish.side_effect = Exception("SNS service unavailable")
        
        with pytest.raises(NotificationError, match="Failed to send error notification"):
            notification_service.send_error_notification("Test error", 'dev')

    def test_send_error_notification_subject_format(self, notification_service, mock_sns_client):
        """Test error notification subject format."""
        mock_sns_client.publish.return_value = {
            'MessageId': 'error-message-id-012'
        }
